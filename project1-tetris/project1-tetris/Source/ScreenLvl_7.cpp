...
ScreenLvl_7::ScreenLvl_7(bool startEnabled) : Module(startEnabled)
{
	// curtain animation
...
ScreenLvl_7::~ScreenLvl_7()
{

}

// Load assets
bool ScreenLvl_7::Start()
{
	App->tetronimo->Enable();

	LOG("Loading lvl 7 background assets");
...
update_status ScreenLvl_7::Update()
{
...
update_status ScreenLvl_7::PostUpdate()
{
	...
	App->render->TextDraw("7", 353, 418, 0, 0, 150, 255, 16);
...
void ScreenLvl_7::lvl_win()
{
	...
	if (v_WinLose >= 574)
	{
		...
		if (openCurtain.GetLoopCount() == 1) { App->render->Blit(curtain_texture, 257, 192, &(closeCurtain.GetCurrentFrame()), 0.85f); }
	...
	if (v_WinLose == 604)		// cambiar (depende del bonus)
	{
		...
		App->fade->FadeToBlack(this, (Module*)App->sLvl_10, 0);
	}
...
void ScreenLvl_7::lvl_lose(const char* ch_loseContinue)
{
