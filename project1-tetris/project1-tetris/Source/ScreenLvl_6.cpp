...
ScreenLvl_6::ScreenLvl_6(bool startEnabled) : Module(startEnabled)
{
	// curtain animation
...
ScreenLvl_6::~ScreenLvl_6()
{

}

// Load assets
bool ScreenLvl_6::Start()
{
	App->tetronimo->Enable();

	LOG("Loading lvl 6 background assets");
...
update_status ScreenLvl_6::Update()
{
...
update_status ScreenLvl_6::PostUpdate()
{
	...
	App->render->TextDraw("6", 353, 418, 0, 0, 150, 255, 16);
...
void ScreenLvl_6::lvl_win()
{
	...
	if (v_WinLose >= 574)
	{
		...
		if (openCurtain.GetLoopCount() == 1) { App->render->Blit(curtain_texture, 258, 192, &(closeCurtain.GetCurrentFrame()), 0.85f); }
	...
	if (v_WinLose == 604)		// cambiar (depende del bonus)
	{
		...
		App->fade->FadeToBlack(this, (Module*)App->sLvl_7, 0);
	}
...
void ScreenLvl_6::lvl_lose(const char* ch_loseContinue)
{
